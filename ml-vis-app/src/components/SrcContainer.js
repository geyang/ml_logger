...
class _ParameterDataContainer extends Component {
    state = {
        records: [],
    };
    ...
    static getDerivedStateFromProps(props, state) {
        const {metricRecords, dataKey} = props;
        const {records} = metricRecords[dataKey] || {};
        if ((!state.records || !state.records.length) && !records) return null;
    ...
    componentDidMount() {
        ...
        const {records} = metricRecords[dataKey] || {};
        if (!records) fetchCallback(dataKey);
    }
    ...
    render() {
        ...
        return <Component records={records} {...props}>{children}</Component>;
    }
}

export const ParameterDataContainer = selector(({metricRecords}) => ({metricRecords}), _ParameterDataContainer);
